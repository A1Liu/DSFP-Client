...
/**
 * This class represents an implementation of the Command interface that parses user input into safely castable objects.
 * 
 * 
 * 
 * @author Alyer
...
public abstract class ParamC implements Command {
	...
	/**
	 * Constructor with a field allowing for some pre-screening of variable types. The screening doesn't care if user enters extra parameters.
	 * @param reqs list of data-types that params should be, in order of user-entry
	 */
	 ...
	 * If the user input isn't one of String, long, float, double, boolean, or int, then this method has to manually parse from string form.
	 * @param elist
	 */
	...
	/**
	 * converts a parameter into an object. Only supports the primitives' object forms -- commands that take separate objects need to parse those separately
	...
	private static final boolean paramCheck(String paramReq, String param) {
		switch(paramReq.toLowerCase()) {
		case "double":
			try {Double.parseDouble(param);return true;} catch (NumberFormatException e) {return false;}
		case "integer":
		case "int":
			try {Integer.parseInt(param);return true;} catch (NumberFormatException e) {return false;}
		case "boolean":
			return param.toLowerCase().equals("true") || param.toLowerCase().equals("false");
		case "float":
			try {Float.parseFloat(param);return true;} catch (NumberFormatException e) {return false;}
		case "long":
			try {Long.parseLong(param);return true;} catch (NumberFormatException e) {return false;}
		default:
			return true;
		}
	}
