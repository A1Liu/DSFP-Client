...
class CommandList {
	...
	private int count;
	private ArrayList<ComTreeNode> nullPoints;
	...
	CommandList() {
		...
		count = 0;
		nullPoints = new ArrayList<ComTreeNode>();
	}
	...
	void addGraph(String[] commands) {
		...
		for (ComTreeNode c : nullPoints) addHelp(c);
	}
	...
	private void addGraph(ComTreeNode node, String[] commands, int startIndex, int endIndex) {
		...
		if (node.getName() == null) nullPoints.add(node);//if the parent node we're looking at is a null, we should add a help menu
	}
	...
	void addHelp(ComTreeNode parent) {
		if (!parent.containsChild("help")) {//Need to do this for all null children as well.
			while (coms.containsKey(++count)) {}
			parent.addChild("help", count,"This command gives helpful information. You can add command paths as arguments and it will give you help for a specific command.");
